...
```{r}
 # Get coeffiencents male
  ...
  c1_aspec_m_eng <- coef(my.seg_male_eng)[[2]] + coef(my.seg_male)[[3]]
...
obs_fit_both_eng %>% 
  ...
  geom_line ( 
    aes (y=predicted)) +
  geom_abline(intercept = b0_aspec_m, slope = b1_aspec_m, color="blue", linetype="dashed") +
    geom_abline(intercept = b0_aspec_f, slope = b1_aspec_f, color="red", linetype="dashed")
